...
async function uploadDir(folder) {
    ...
    try {
        ...
        const res = await bundler.uploader.uploadFolder(folder, (_a = options.indexFile) !== null && _a !== void 0 ? _a : null, +options.batchSize, options.confirmation, console.log);
        if (res != "none") {
...
async function init(opts, operation) {
    ...
    try {
        ...
        bundler = new bundlr_1.default(opts.host, opts.currency.toLowerCase(), wallet);
    }
    catch (err) {
